...
public class PokerTable extends Observable {
  ...
  public PokerTable(int numberOfSeats, int dealer) {
    this.dealer = dealer;
  ...
  public void setNumberOfSeats(int numberOfSeats) {
    if (numberOfSeats < players.size()) {
      ...
      for(int index = numberOfSeats - players.size(); index--; index > 0)
        players.add(null);
      doNotify();
    }
  }
   ...
   * unavailable.
   */
  public void addPlayer(int seat, PlayerInfo player) {
    if (isOccupied(seat))
      return false;
